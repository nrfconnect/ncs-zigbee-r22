...
See `Image-specific variables`_ to learn how to set the required options.
...
For instructions on how to set these additional options and configuration at build time, see `Providing CMake options`_ and `Configuring an application`_.
...
See the Testing section of the `USB DFU (Device Firmware Upgrade)`_ for the list of required dfu-util commands.

To learn more about configuring bootloader for an application in |NCS|, see the `Secure bootloader chain`_ page.
