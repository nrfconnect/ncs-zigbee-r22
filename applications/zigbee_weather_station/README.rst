...
.. include:: /includes/device_table_weather_station.txt
...
See `Custom configurations`_ and `CMake options <Providing CMake options_>`_ for more information.
...
See `CMake options <Providing CMake options_>`_ for information about how to select a suffixed configuration.
...
1. ``debug`` configuration: Connect the device to the computer using a USB cable.
...
#. Connect to the device with a terminal emulator (for example, the `Serial Terminal app`_).
